...
class QPBFT_Node(Node):
    ...
    def __repr__(self) -> str:
        return f"IdUser => {self.idUser},Score => {self.reliable_score:.2f}, Role => {self.role.value:>8}, Faulty => {str(self.faulty):>6}, Send_message_log => {str(self.send_message_log[:-1] if self.send_message_log else "\'\'"):<35}, Receive_messages_log => {self.messages_log}"
